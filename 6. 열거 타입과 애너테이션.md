...
클래스의 최상위 조상인 Object의 isEquals를 재정의하기 위해선 매개변수로 Object 타입을 받아야한다. 그렇지 않을 경우, 재정의가 아닌 다중정의가 되어버린다. 따라서 Override 애너테이션이 붙지 않은 isEquals는 동작하지 않고, Ojbect의 isEquals가 동작하여 알파벳 문자가 같은 객체라도 주소값이 다르기 떄문에 다른 객체로 여겨진다.
```java
